...
# trainer.py, _train 方法中的 while 循环

# ...

                    # 1. 记录移动前的信息
                    prev_pos = game.board.components['Player'].pos
                    prev_heading = game.board.components['Player'].heading

                    # 2. 执行移动
                    game.move(action_) 

                    # 3. 检查是否撞墙/卡住
                    curr_pos = game.board.components['Player'].pos
                    collided_or_stuck = (prev_pos == curr_pos) 

                    # 4. 获取奖励
                    # 新的 get_reward 函数现在只计算当前步骤的稠密奖励
                    reward = game.get_reward(action_, prev_heading, collided_or_stuck)
                    
                    # 5. 统一处理所有终止条件
                    is_success = (curr_pos == exit_pos and not collided_or_stuck)
                    is_failure = (
                        collided_or_stuck or
                        moves + 1 >= self.config["max_moves"] or
                        game.energy_model.is_depleted()
                    )
                    done = is_success or is_failure

                    # 6. 根据最终状态，覆写最终奖励 (给予稀疏的最终奖惩)
                    if is_success:
                        reward = 100.0  # 巨大的成功奖励
                        status = 0
                    elif is_failure:
                        reward = self.config.get('penalty', -10.0) # 巨大的失败惩罚
                        status = 0
                    
                    # 7. 获取新的状态并储存经验
                    state2_ = game.board.render_np().reshape(1, self.in_features)
                    state2 = torch.from_numpy(state2_).float()
